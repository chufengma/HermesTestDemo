...
JSIExecutor::JSIExecutor(
  ...
  runtime_->global().setProperty(
    ...
    // CRN BEGIN
    {
      setGlobalVariable("CRNGlobal", folly::make_unique<JSBigStdString>("{}"));
    }
    // CRN END
}
...
//CRN BEGIN
    void JSIExecutor::loadModuleForCRN(std::string &modulePath, std::string &moduleDiffPath, std::string &moduleId) {
    ...
    std::string JSIExecutor::readScriptFromFileForCRN(const std::string& solidFile, const std::string &diffFile) {
        ...
        if (code == "") {
            ...
            runtime_->evaluateJavaScript(std::make_unique<StringBuffer>(code), filePath);
        }
        return "{}";
    }
    //CRN END
      ...
      if (!moduleId.empty()) {
          try {
              if (m_isBindCRNNativeRequire && m_CRNJSModuleConfig.size() > 1) {
                  ...
                  loadModuleForCRN(moduleFilePath, moduleDiffFilePath, moduleId);

                  // LOG(INFO) << "nativeRequire require module: " << moduleFilePath << "  diff:" << moduleDiffFilePath;
              } else {
                  uint32_t moduleIdInt = folly::to<uint32_t>(args[0].getNumber());
              ...
              LOG(ERROR) << "nativeRequire exception, module: " << moduleId;
