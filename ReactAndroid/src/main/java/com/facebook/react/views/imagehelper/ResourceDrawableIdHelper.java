...
// CRN BEGIN
  // global CRNLocalSourceHandler

  private static CRNLocalSourceHandler mCRNLocalSourceHandler;

  public static void setCRNLocalSourceHandler(CRNLocalSourceHandler CRNLocalSourceHandler) {
    ResourceDrawableIdHelper.mCRNLocalSourceHandler = CRNLocalSourceHandler;
  }
  ...
  public int getResourceDrawableId(Context context, @Nullable String name) {
    ...
    synchronized (this) {
      ...
      // CRN BEGIN
      // image source from crn handler or default hander

      int id = 0;
      if (mCRNLocalSourceHandler == null) {
        id = context.getResources().getIdentifier(name, "drawable", context.getPackageName());
      } else {
        id = mCRNLocalSourceHandler.resolveResourceId(context, name);
      }
