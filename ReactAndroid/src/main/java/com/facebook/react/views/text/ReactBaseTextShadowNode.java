  ...
  private static void buildSpannedFromShadowNode(
    ...
    for (int i = 0, length = textShadowNode.getChildCount(); i < length; i++) {
      ...
      if (child instanceof ReactRawTextShadowNode) {
        String text = ((ReactRawTextShadowNode) child).getText();
        sb.append(
                TextTransform.apply(
                        text == null ? "" : text,
    ...
    if (end >= start) {
      ...
      if (textShadowNode.mFontStyle != UNSET
        ...
        // CRN BEGIN
        CatalystInstance instance = textShadowNode.getThemedContext().getCatalystInstance();
        final String modulePath = instance.getModulePath();
