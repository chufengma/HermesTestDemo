...
public class UIViewOperationQueue {
  ...
  //CRN BEGIN
  private CRNUIEventListener uiEventListener;
  private boolean postEventWhenUIChangedFlag = false;

  public void notifyEventListener() {
    if (postEventWhenUIChangedFlag && uiEventListener != null) {
      uiEventListener.onViewUpdated();
      postEventWhenUIChangedFlag = false;
    }
  }

  public void setUIRenderEventListener(CRNUIEventListener eventListener, boolean postEventWhenUIChangedFlag) {
    this.uiEventListener = eventListener;
    this.postEventWhenUIChangedFlag = postEventWhenUIChangedFlag;
  }

  public void clearViews() {
    mNativeViewHierarchyManager.clearViews();
  }
  //CRN END
  /**
  ...
  private final class MeasureOperation implements UIOperation {
    ...
    public void execute() {
      try {
        if (!mNativeViewHierarchyManager.measure(mReactTag, mMeasureBuffer)) {
          mCallback.invoke();
          return;
        }
  ...
  private final class FindTargetForTouchOperation implements UIOperation {
    ...
    public void execute() {
      try {
        if (!mNativeViewHierarchyManager.measure(
                mReactTag,
                mMeasureBuffer)) {
          mCallback.invoke();
          return;
